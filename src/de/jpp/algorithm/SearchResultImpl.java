...
public class SearchResultImpl<N, A> implements ObservableSearchResult<N, A> {
    ...
    public Optional<List<Edge<N, A>>> getPathTo(N dest) {
        ...
        if(statusMap.get(dest) != NodeStatus.CLOSED){
            System.out.println("Debugging Pfad, Ziel nicht geschlossen: " + dest);
        ...
        while (cur != null){
            System.out.println("Debugging, Zurückverfolgen von: " + cur.getDestination() + " zu " + cur.getStart());
            edges.add(cur);
    ...
    public void open(N node, NodeInformation<N, A> info){
        System.out.println("Debugging, Oeffnen des Knotens " + node);
        infoMap.put(node, info);
        setOpen(node);
    }

    public void close(N node, NodeInformation<N, A> info){
        System.out.println("Debugging, Schliessen des Knotens: " + node);
        infoMap.put(node, info);
        setClosed(node);
    }
    ...
    public void setStatusUnknown(N node){
